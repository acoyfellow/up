...
type ComposerSession =
  | { state: 'none' }
  | { state: 'active' | 'expired'; liveUrl: string; expiresAt: string };
...
function inspectionHtml(
  ...
  const sessionMarkup =
    session.state === 'none'
      ? '<div id="session" class="session none"><strong>No local deployment yet.</strong><span>Inspecting this project has not created an account.</span></div>'
      : `<div id="session" class="session ${session.state}" data-expires-at="${escapeHtml(session.expiresAt)}"><strong>${session.state === 'active' ? 'Temporary app is active.' : 'Temporary app expired.'}</strong><a href="${escapeHtml(session.liveUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(session.liveUrl)}</a><span id="session-expiry">${session.state === 'active' ? 'Calculating time remaining…' : 'Deploy again to create a fresh Temporary Account.'}</span></div>`;
...
:root{color-scheme:light;--ink:#111923;--muted:#5f6f7c;--line:#d7dde1;--paper:#f5f7f8;--orange:#f6821f;--blue:#2678a4;font:16px/1.55 system-ui,sans-serif}*{box-sizing:border-box}.skip{position:fixed;z-index:2;top:8px;left:8px;padding:8px;background:#fff;color:var(--ink);transform:translateY(-150%)}.skip:focus{transform:none}body{margin:0;background:#fff;color:var(--ink)}main{width:min(1080px,calc(100% - 32px));margin:auto;padding:42px 0 70px}header{padding:28px 0 32px;border-bottom:1px solid var(--line)}.eyebrow{color:var(--orange);font:600 .72rem ui-monospace,monospace;letter-spacing:.08em;text-transform:uppercase}h1{max-width:760px;margin:12px 0 10px;font-size:clamp(2.2rem,6vw,4.5rem);line-height:1;letter-spacing:-.045em}header p{max-width:720px;color:var(--muted);overflow-wrap:anywhere}.warnings{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:24px 0}.warning{padding:16px;border:1px solid var(--line);border-left:4px solid var(--orange);border-radius:5px;background:var(--paper)}.warning.safe{border-left-color:#16855b}.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}.card{min-width:0;padding:22px;border:1px solid var(--line);border-radius:7px;overflow-wrap:anywhere}.card h2{margin:0 0 14px;font-size:1rem}.card ul{max-height:240px;margin:0;padding-left:20px;overflow:auto}.card li{margin:6px 0}.empty{color:var(--muted)}code{font-family:ui-monospace,monospace;font-size:.82em;overflow-wrap:anywhere}.plan{margin-top:16px;padding:22px;border-radius:7px;background:#0b1118;color:#fff}.plan code{display:block;overflow:auto;color:#9fd7ef;white-space:pre}.session{display:grid;gap:5px;margin:16px 0;padding:16px;border:1px solid var(--line);border-left:4px solid var(--blue);border-radius:5px;background:var(--paper);overflow-wrap:anywhere}.session.expired{border-left-color:#c73b31}.session.active{border-left-color:#16855b}.session a{color:var(--blue)}.deploy{margin-top:16px;padding:22px;border:1px solid var(--line);border-radius:7px}.consent{display:grid;gap:10px;margin:16px 0;padding:0;border:0}.consent legend{margin-bottom:8px;font-weight:700}.consent label{display:flex;align-items:flex-start;gap:9px}.consent input{margin-top:.3em}button{min-height:44px;padding:0 18px;border:0;border-radius:5px;background:var(--orange);color:#1d1009;font-weight:750;cursor:pointer}button:focus-visible,input:focus-visible,textarea:focus-visible,a:focus-visible{outline:3px solid #71b8d8;outline-offset:2px}button:disabled{opacity:.45;cursor:not-allowed}.progress{margin-top:16px;padding:16px;min-height:100px;max-height:320px;overflow:auto;border-radius:5px;background:#0b1118;color:#dce7ed;font:12px/1.5 ui-monospace,monospace;white-space:pre-wrap}.result{margin-top:14px;padding:16px;border-left:4px solid #16855b;background:var(--paper);overflow-wrap:anywhere}.result.partial{border-left-color:#c77b18}.result a{color:var(--blue);font-weight:700}.result-actions{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}.secondary{border:1px solid var(--line);background:#fff;color:var(--ink)}.checks{display:grid;gap:7px;margin:12px 0}.check{padding:9px;border:1px solid var(--line);border-radius:4px}.check.pass{border-left:4px solid #16855b}.check.fail{border-left:4px solid #c73b31}.handoff{width:100%;min-height:130px;margin-top:12px;padding:10px;border:1px solid var(--line);font:12px/1.45 ui-monospace,monospace}.meta{display:flex;flex-wrap:wrap;gap:8px;margin-top:18px}.meta span{padding:5px 8px;border:1px solid var(--line);border-radius:999px;color:var(--muted);font-size:.72rem}@media(max-width:700px){main{width:min(100% - 24px,1080px);padding-top:18px}.grid,.warnings{grid-template-columns:1fr}.card,.deploy,.plan{padding:16px}.result-actions{display:grid}.result-actions button{width:100%}.progress,.plan code{max-width:100%;overflow:auto}.handoff{min-height:180px}}@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;transition:none!important;animation:none!important}}
</style></head><body><a href="#deploy" class="skip">Skip to deploy</a><main><header><div class="eyebrow">Local inspection · no account created</div><h1>${escapeHtml(inspection.workerName)}</h1><p>${escapeHtml(inspection.projectRoot)}</p><div class="meta"><span>${inspection.layout} layout</span><span>${inspection.assets.length} public assets</span><span>${inspection.workerModules.length} Worker modules</span><span>${bindings.length} bindings</span></div></header>${sessionMarkup}<section class="warnings"><div class="warning"><strong>Public and temporary</strong><br>The app and API will be public for about an hour. Do not deploy secrets or private data.</div><div class="warning safe"><strong>Your accounts stay isolated</strong><br>Up launches Wrangler in a project-only home and removes inherited Cloudflare credentials.</div></section><section class="grid"><div class="card"><h2>Public assets</h2>${list(inspection.assets, 'No public assets.')}</div><div class="card"><h2>Worker modules</h2>${list(inspection.workerModules, 'Static-only project.')}</div><div class="card"><h2>Bindings</h2>${list(bindings, 'No platform bindings.')}</div><div class="card"><h2>Health checks</h2>${list(checks, 'Only the public page check will run.')}</div><div class="card"><h2>Excluded</h2>${list(inspection.excluded, 'Nothing excluded.')}</div></section><section class="plan"><strong>Command plan</strong><code>${escapeHtml(inspection.command)}</code></section><section id="deploy" class="deploy"><h2>Deploy this app</h2><p>Up will run the pinned CLI locally and stream redacted Wrangler progress here.</p><form id="deploy-form"><fieldset class="consent"><legend>Required approvals</legend><label><input id="public-consent" type="checkbox" required> I understand the app and API will be public.</label><label><input id="terms-consent" type="checkbox" required> I accept Cloudflare&rsquo;s Terms of Service and Privacy Policy.</label></fieldset><button id="deploy-button" type="submit" disabled>Deploy temporary app</button></form><pre id="progress" class="progress" role="status" aria-live="polite" aria-atomic="false">Waiting for approval.</pre><div id="result" class="result" tabindex="-1" hidden><strong>App is live</strong><br><a id="live-url" target="_blank" rel="noopener noreferrer"></a><div id="checks" class="checks"></div><div class="result-actions"><button id="ownership-button" type="button">Open ownership</button><button id="copy-button" class="secondary" type="button">Copy agent handoff</button></div><textarea id="handoff" class="handoff" readonly>${escapeHtml(handoff)}</textarea><p id="action-status" aria-live="polite"></p></div></section></main><script nonce="${nonce}">const deploySection=document.querySelector('#deploy'),form=document.querySelector('#deploy-form'),publicConsent=document.querySelector('#public-consent'),termsConsent=document.querySelector('#terms-consent'),button=document.querySelector('#deploy-button'),progress=document.querySelector('#progress'),result=document.querySelector('#result'),liveUrl=document.querySelector('#live-url'),checks=document.querySelector('#checks'),ownershipButton=document.querySelector('#ownership-button'),copyButton=document.querySelector('#copy-button'),handoff=document.querySelector('#handoff'),actionStatus=document.querySelector('#action-status'),session=document.querySelector('#session'),sessionExpiry=document.querySelector('#session-expiry');let expiryTimer;const updateButton=()=>button.disabled=deploySection.getAttribute('aria-busy')==='true'||!(publicConsent.checked&&termsConsent.checked);const showExpiry=(expiresAt,target)=>{if(!target||!expiresAt)return;clearInterval(expiryTimer);const render=()=>{const remaining=Math.max(0,Date.parse(expiresAt)-Date.now());if(!remaining){target.textContent='Expired. Deploy again to create a fresh Temporary Account.';session?.classList.remove('active');session?.classList.add('expired');clearInterval(expiryTimer);return}const minutes=Math.ceil(remaining/60000);target.textContent='Expires in about '+minutes+' minute'+(minutes===1?'':'s')+'.'};render();expiryTimer=setInterval(render,30000)};if(session?.dataset.expiresAt&&sessionExpiry)showExpiry(session.dataset.expiresAt,sessionExpiry);publicConsent.addEventListener('change',updateButton);termsConsent.addEventListener('change',updateButton);const events=new EventSource('./events');events.onerror=()=>{actionStatus.textContent='Progress connection interrupted; reconnecting…'};events.onopen=()=>{if(actionStatus.textContent.includes('reconnecting'))actionStatus.textContent='Progress reconnected.'};events.onmessage=(message)=>{const event=JSON.parse(message.data);if(event.type==='log'){progress.textContent+=(progress.textContent==='Waiting for approval.'?'':'\n')+event.data;progress.scrollTop=progress.scrollHeight}if(event.type==='check'){const item=document.createElement('div');item.className='check '+(event.passed?'pass':'fail');item.textContent=(event.passed?'✓ ':'✗ ')+event.name+' · '+event.detail;checks.append(item)}if(event.type==='result'){liveUrl.href=event.liveUrl;liveUrl.textContent=event.liveUrl;result.hidden=false;result.classList.toggle('partial',!event.checksPassed);result.querySelector('strong').textContent=event.checksPassed?'App is live and checks passed':'App is live; a check failed';deploySection.setAttribute('aria-busy','false');button.disabled=true;button.textContent='Deployed';showExpiry(event.expiresAt,actionStatus);result.focus()}if(event.type==='error'){progress.textContent+='\nError: '+event.data;deploySection.setAttribute('aria-busy','false');button.textContent='Retry';updateButton();button.focus()}};form.addEventListener('submit',async(event)=>{event.preventDefault();deploySection.setAttribute('aria-busy','true');button.disabled=true;button.textContent='Deploying…';progress.textContent='Starting pinned Up CLI…';checks.replaceChildren();result.hidden=true;result.classList.remove('partial');actionStatus.textContent='';const response=await fetch('./deploy',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({acceptPublic:publicConsent.checked,acceptTerms:termsConsent.checked})});if(!response.ok){progress.textContent='Deploy refused: '+await response.text();deploySection.setAttribute('aria-busy','false');button.textContent='Retry';updateButton();button.focus()}});ownershipButton.addEventListener('click',async()=>{ownershipButton.disabled=true;const response=await fetch('./ownership',{method:'POST',headers:{'content-type':'application/json'},body:'{}'});actionStatus.textContent=response.ok?'Ownership flow opened in your browser.':await response.text();ownershipButton.disabled=false;ownershipButton.focus()});copyButton.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(handoff.value)}catch{handoff.select();document.execCommand('copy')}actionStatus.textContent='Agent handoff copied.';copyButton.focus()});</script></body></html>`;
}
...
async function openComposer(): Promise<void> {
  ...
  const projectPaths = anonymousPaths(root);
  const [existingMetadata, existingAccount] = await Promise.all([
    readProjectMetadata(projectPaths),
    readTemporaryAccount(projectPaths, true).catch(() => null),
  ]);
  const session: ComposerSession =
    existingMetadata && existingAccount
      ? {
          state: Date.parse(existingAccount.accountExpiresAt) > Date.now() ? 'active' : 'expired',
          liveUrl: existingMetadata.liveUrl,
          expiresAt: existingAccount.accountExpiresAt,
        }
      : { state: 'none' };
  ...
  const server = http.createServer((request, response) => {
    ...
    if (request.method === 'GET' && request.url === pagePath) {
      ...
      response.end(inspectionHtml(inspection, nonce, session));
      return;
    }
    ...
    if (request.method === 'POST' && request.url === deployPath) {
      ...
      request.on('end', () => {
        ...
        child.once('exit', async (code) => {
          ...
          const account = await readTemporaryAccount(projectPaths);
          ...
          emit({
            ...
            expiresAt: account.accountExpiresAt,
