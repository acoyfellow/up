...
import type { SiteSecrets } from './site-secrets';
...
export function isCoreRequest(request: Request, env: Env): boolean {
  ...
  const siteHostname = Boolean(
    domain && host !== domain && host !== controlHost && host.endsWith(`.${domain}`),
  );
  return siteHostname || url.pathname.startsWith('/api/') || url.pathname === '/app/__session';
}
