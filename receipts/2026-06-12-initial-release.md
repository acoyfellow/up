# Up 0.0.1 initial release receipt
...
- Repository: https://github.com/acoyfellow/up
- Documentation and demo: https://up.coey.dev
- Deploy button: redirects to Cloudflare Dashboard with `acoyfellow/up`
- Docs Worker: `up-site`, version `0826bedc-da85-443d-aae6-a79442efde29`
...
The private R2 bucket was provisioned. A personal-account `up-app` Worker was used to verify production routing behavior: its attempted route deployment returned only fail-closed 503 responses. Because the available API token cannot create Access applications, the Worker and every app/site route were then deleted rather than leave a binding-bearing public target. `workers.dev` and Preview URLs were never enabled.

This is not yet an authenticated production receipt. The remaining gate is to create the personal account's Access organization/application, configure the exact issuer/audience, attach `app.up.coey.dev` plus `*.up.coey.dev`, then execute the authenticated publish and isolated anonymous-denial checks. No public fallback will be used to bypass that gate.
